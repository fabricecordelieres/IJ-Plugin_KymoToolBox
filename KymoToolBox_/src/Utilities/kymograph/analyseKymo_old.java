...
public class analyseKymo_old {
    ...
    boolean logFullData;
    /**Contains one pathInfo_old object per track to analyse**/
    ArrayList<pathInfo_old> tracks=null;
     ...
     */
    public analyseKymo_old(ImagePlus ip, Roi[] rois, boolean direction, double minSpeed, boolean logFullData){
        this.ip=ip;
     ...
     * @param logFullData true if all the data have to be logged (individual distance/speed for each track segment). If false, only a sumamry will be logged.
     */
    public analyseKymo_old(ImagePlus ip, boolean direction, double minSpeed, boolean logFullData){
    ...
    public ImagePlus getImage(int lineWidth){
        ...
        if (tracks.size()!=0){
            ...
            for (int i=0; i<tracks.size(); i++){
                pathInfo_old pi=(pathInfo_old) tracks.get(i);
        ...
        if (!tracks.isEmpty()){
            ...
            for (int i=0; i<tracks.size(); i++){
                ...
                for (int j=0; j<pi.nbPointsFullPath; j++){
                    ...
                    out.setPosition(1, pi.yFullPath[j]+1, 1);
                    out.getChannelProcessor().drawDot(xPath[pi.xFullPath[j]], yPath[pi.xFullPath[j]]);
               }
            }
            out.resetDisplayRanges();
        ...
        if (tracks.size()!=0){
            ...
            for (int i=0; i<tracks.size(); i++){
                ...
                for (int j=0; j<pi.nbPointsFullPath; j++){
                    ...
                    rt.setValue("x", row, xPath[pi.xFullPath[j]]);
                    rt.setValue("y", row, yPath[pi.xFullPath[j]]);

                    double dist=j==0?Double.NaN:cal.pixelWidth*Math.sqrt((xPath[pi.xFullPath[j]]-xPath[pi.xFullPath[j-1]])*(xPath[pi.xFullPath[j]]-xPath[pi.xFullPath[j-1]])+(yPath[pi.xFullPath[j]]-yPath[pi.xFullPath[j-1]])*(yPath[pi.xFullPath[j]]-yPath[pi.xFullPath[j-1]]));
        ...
        if (tracks.size()!=0){
            ...
            for (int i=0; i<tracks.size(); i++){
                ...
                for (int j=0; j<pi.nbPointsFullPath; j++){
                    ...
                    if (dist!=0) dist=pi.mvtFullPath[j]==1?-dist:dist;
                    double speed=dist/cal.frameInterval;
                    out.add(new String[]{ip.getTitle(), (i+1)+"", pi.timeFullPath[j]+"", xPath[pi.xFullPath[j]]+"", yPath[pi.xFullPath[j]]+"", dist+"", speed+""});
                }
            }
        }
        return out;
    ...
    public void logResults(boolean logFullData){
        tracks=new ArrayList<pathInfo_old>();
        for (int i=0; i<rois.length; i++){
            ...
            if ((type==Roi.LINE || type==Roi.POLYLINE) && rois[i].getBounds().getHeight()!=0.0){
                tracks.add(new pathInfo_old(rois[i], dir, cal, minSpeed));
                if (((pathInfo_old) tracks.get(tracks.size()-1)).back2theFuture) tracks.remove(tracks.size()-1);
            }else{
                IJ.log("Roi "+rois[i].getName()+" is not a line or a polyline  Roi, or is a horizontal: no analysis will be done on that track.");
            }
        }
        for (int i=0; i<tracks.size(); i++) ((pathInfo_old) tracks.get(i)).logData(i+1, ip.getTitle(), logFullData);
    }
    ...
    public ArrayList<String[]> getResults(boolean logFullData){
        ...
        for (int i=0; i<tracks.size(); i++){
        	ArrayList<String[]> currResults=((pathInfo_old) tracks.get(i)).getData(i+1, ip.getTitle(), logFullData);
    ...
    private void fillPathInfo(){
        ...
        if (infoPath!=null && infoPath.indexOf("<KymopathInfo_old>")!=-1){
