...
public class pathInfo_old {
     ...
     */
    public pathInfo_old(Roi roi, boolean direction, Calibration cal, double minSpeed){
        dir=direction?-1:1;
        this.cal=cal;
        ...
        if (back2theFuture){
            ...
            xFullPath=new int[nbPointsFullPath];
            yFullPath=new int[nbPointsFullPath];
            ...
            for (int i=0; i<nbPoints-1; i++){
                ...
                for (int j=y[i]; j<y[i+1]; j++){
                    xFullPath[index]=(Double.isNaN(a))?x[i]:(int) ((j-b)/a+.5);
                    yFullPath[index]=j;
